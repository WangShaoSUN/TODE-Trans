...
class ClearGraspRealWorld:
    """
    ClearGrasp real-world dataset for MindSpore.
    """
    def __init__(self, data_dir, split='test', **kwargs):
        ...
        Parameters
        ----------
        data_dir: str, the data path;
        split: str, the dataset split option, only 'test' is supported.
        """
        if split not in ['test']:
            raise AttributeError('Cleargrasp real-world dataset is only supported for testing.')
        ...
        assert len(self.image_paths) == len(self.mask_paths) == len(self.depth_paths) == len(self.depth_gt_paths)
