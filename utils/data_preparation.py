...
def process_data(
    ...
    **kwargs):
    ...
    data_dict = {
        'rgb': Tensor(rgb, dtype=mnp.float32),
        'depth': Tensor(depth, dtype=mnp.float32),
        'depth_gt': Tensor(depth_gt, dtype=mnp.float32),
        'depth_gt_mask': Tensor(depth_gt_mask, dtype=mnp.bool_),
        'scene_mask': Tensor(scene_mask, dtype=mnp.bool_),
        'zero_mask': Tensor(neg_zero_mask, dtype=mnp.bool_),
        'zero_mask_dilated': Tensor(neg_zero_mask_dilated, dtype=mnp.bool_),
        'initial_loss_mask': Tensor(initial_loss_mask, dtype=mnp.bool_),
        'initial_loss_mask_dilated': Tensor(initial_loss_mask_dilated, dtype=mnp.bool_),
        'loss_mask': Tensor(loss_mask, dtype=mnp.bool_),
        'loss_mask_dilated': Tensor(loss_mask_dilated, dtype=mnp.bool_),
        'depth_original': Tensor(depth_original, dtype=mnp.float32),
        'depth_gt_original': Tensor(depth_gt_original, dtype=mnp.float32),
        'depth_gt_mask_original': Tensor(depth_gt_mask_original, dtype=mnp.bool_),
        'zero_mask_original': Tensor(zero_mask_original, dtype=mnp.bool_),
        'fx': Tensor(camera_intrinsics[0, 0], dtype=mnp.float32),
        'fy': Tensor(camera_intrinsics[1, 1], dtype=mnp.float32),
        'cx': Tensor(camera_intrinsics[0, 2], dtype=mnp.float32),
        'cy': Tensor(camera_intrinsics[1, 2], dtype=mnp.float32)
    }

    # Calculate surface normals (ensure compatibility with MindSpore)
    data_dict['depth_gt_sn'] = get_surface_normal_from_depth(
        Tensor(data_dict['depth_gt'].unsqueeze(0)), 
        Tensor(data_dict['fx'].unsqueeze(0)), 
        Tensor(data_dict['fy'].unsqueeze(0)), 
        Tensor(data_dict['cx'].unsqueeze(0)), 
        Tensor(data_dict['cy'].unsqueeze(0))
    ).squeeze(0)
